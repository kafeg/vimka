    ...
    Package {
        ...
        Item {
            ...
            VisualDataModel {
                ...
                //model: VKXmlModel { id: vkModel; albTitle: title; albUrl: ""; }
                model: VKXmlModel { id: vkModel; albTitle: title; albUrl: url; }
            }

            BusyIndicator {
                id: busyIndicator
                anchors { centerIn: parent; verticalCenterOffset: -20 }
                on: vkModel.status != XmlListModel.Ready
            }

            PathView {
                id: photosPathView; model: visualModel.parts.stack; pathItemCount: 3
            ...
            MouseArea {
                anchors.fill: parent
                onClicked: { 
                  mainWindow.editMode ? photosModel.remove(index) : albumWrapper.state = 'inGrid'
                  //if (vkModel.source == "" ) { vkModel.source = url }
                }
            }

            Tag {
                anchors { horizontalCenter: parent.horizontalCenter; bottom: parent.bottom; bottomMargin: 10 }
                frontLabel: title; backLabel: qsTr("Remove"); flipped: mainWindow.editMode
                onTagChanged: vkModel.tags = title
